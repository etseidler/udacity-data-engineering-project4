...
## ignored per https://knowledge.udacity.com/questions/990597
# COPY_MONTHLY_TRIPS_SQL = COPY_SQL.format(
#     "trips",
#     "s3://<S3_BUCKET_NAME>/data-pipelines/divvy/partitioned/{year}/{month}/divvy_trips.csv"
# )

COPY_ALL_TRIPS_SQL = COPY_SQL.format(
    "trips",
    "s3://<S3_BUCKET_NAME>/data-pipelines/divvy/unpartitioned/divvy_trips_2018.csv"
)

COPY_STATIONS_SQL = COPY_SQL.format(
    "stations",
    "s3://<S3_BUCKET_NAME>/data-pipelines/divvy/unpartitioned/divvy_stations_2017.csv"
)

## unused: broken into three separate statements later, in-line, in DAG code s3_to_redshift.py
